...
public class UsuarioController {
    ...
    public ResponseEntity<UsuarioDTO> registrar(
            @Valid @ModelAttribute UsuarioRegistroDTO registroDTO) throws Exception {
...
@GetMapping("/email/{email}")
    ...
    public ResponseEntity<UsuarioDTO> actualizarFoto(
            ...
            @RequestParam("foto") MultipartFile nuevaFoto) throws Exception {
    ...
    @GetMapping("/uid/{uid}")
    public ResponseEntity<UsuarioDTO> obtenerPorUid(@PathVariable String uid) {
        UsuarioDTO usuarioDTO = usuarioService.obtenerPorUid(uid);
        return ResponseEntity.ok(usuarioDTO);
    }
